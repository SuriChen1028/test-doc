...
( b_mat, y_mat) = np.meshgrid( b, y, indexing = 'ij')
...
def falseTransient( ell, epsilon,tolerance, stateVariable, stateSteps, stateSpace, params):
    ...
    while episode == 0 or FC_Err > tol:
        ...
        v0_db = finiteDiff(v0,0,1,hb)
        v0_dbb = finiteDiff(v0,0,2,hb)
        ...
        e[e<0] = 1e-300
        # h2 = - v0_dz*np.sqrt(z)*sigma2_100/(xi_m*b_mat)
        print(np.min(e))
        # HJB coefficient
        ...
        D = b_mat*( delta * eta * np.log(e) - delta*(1-eta)*y_mat*z - .5*y_mat**2*z**2 ) - e*ell

        print('here')
        out = PDESolver_2d(stateSpace, A, B_b, B_y, C_bb, C_yy, D, v0,
                           epsilon, solverType = 'False Transient')

        out_comp = out[2].reshape(v0.shape,order = "F")

        PDE_rhs = A*v0 + B_b*v0_db +  B_y*v0_dy + C_bb*v0_dbb + C_yy*v0_dy + D
...
def one_ell(ell, parameters):
    [b_mat, y_mat], [hb, hy], stateSpace, params = parameters
    solution = falseTransient(ell, .5, 1e-8, stateVariable = [b_mat, y_mat], stateSteps = [hb, hy], stateSpace = stateSpace,  params = params)
    return solution
...
start = time.time()
parameters = ([b_mat, y_mat], [hb, hy], stateSpace, params)
...
# restore results
dataFile = '../data/solution/solu_modified_5*50*5_10_0205'
